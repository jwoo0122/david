...
- The public Cargo package and Homebrew formula identity MUST be `david`.
...
Every qualifying push can add a bot-authored commit to `main`. A failed package or tap publication can leave a GitHub Release partially complete. Crates.io publication can be retried with the manual `Publish crate` workflow for the existing release commit and version. The package, release commit, tag, and published artifacts remain aligned by the release workflow.

## Enforcement

CI configuration checks MUST verify the release-plz filter, `david` package identity, version-bump commit format, tag/ref handoff, secret names, target list, Homebrew tap path, and crate recovery workflow. The ordinary Rust test, format, clippy, and locked release-build checks MUST remain green.
