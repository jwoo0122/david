...
`prompt <worktree> <message>` MUST resolve the same managed worktree and session identity as `run`, require a started live agent with matching session metadata, and deliver the exact received message as literal UTF-8 paste data followed by one `Enter` submission. It MUST NOT create a worktree or session, invoke the agent picker, or attach to tmux. Delivery MUST target the exact managed session and agent pane rather than the caller's current tmux target; prompt data MUST travel through tmux stdin and MUST NOT be interpreted as shell commands or tmux key names.
...
A managed session MUST explicitly style only its status line. It MUST NOT set `window-style`, `window-active-style`, `pane-border-style`, or `pane-active-border-style`, so application foregrounds, backgrounds, and pane borders retain tmux defaults. The status line MUST use an achromatic (grayscale) palette with a light background and dark text, and zones MUST be distinguished by background color (lighter at edges, darker toward center) rather than pipe (`|`) separators.
...
- A prepared session with a live pane MAY be reattached to start its agent; a started session MUST be considered live for reuse only while its managed agent pane is live. Dead panes MUST be rejected and prepared sessions MUST be reported as inactive by `list`.
...
- Explicit attach MUST require an existing live managed session and MAY start only the persisted selected agent of a prepared session.
...
- New session metadata MUST retain the prepared pane identity and selected command arguments so first attach and later prompts do not depend on the caller's current pane or current configuration file.
- Prepared or dead agent panes MUST not receive prompts or be reported as active agents.
...
- A managed session MUST restrict explicit color styling to the status line and MUST leave application-area window and pane styles unset.
- Interactive worktree selection from `run` without a name MUST reuse the run flow for an existing worktree and MUST create a new worktree only through the "New worktree..." prompt.
- `list` MUST NOT present an interactive picker; it MUST always show the table or porcelain output.
